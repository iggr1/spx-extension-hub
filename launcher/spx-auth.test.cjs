...
(async () => {
  ...
  assert.equal(window.HubAuth.access('spx-dock-flow').allowed, true);
  assert(calls.every(call => call.operation === 'network.fetchBatch'), 'Authentication must execute as launcher and never impersonate another module');
  console.log('Catalog checks passed: launcher profile auth, pending access, allowed account, OPS denied, Dock Flow public.');
