...
const context = vm.createContext({ document, window, CustomEvent: class {}, localStorage: { removeItem() {} }, sessionStorage: { removeItem() {} }, setInterval() {}, Date, console,
  LoaderBridge: { requestForModule: async (id, operation, payload) => {
    calls.push(operation); await ready;
    assert.equal(id, 'assistente-de-devolucoes');
...
(async () => {
  ...
  assert.equal(window.HubAuth.access('spx-dock-flow').allowed, true);
  assert(calls.every(op => op === 'network.fetchBatch'), 'Authentication must never register or disable user scripts');
  console.log('Catalog checks passed: pending access, allowed account, OPS denied, Dock Flow public, no registration changes.');
